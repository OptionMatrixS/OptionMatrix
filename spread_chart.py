...
def _init():
    for k, v in [
        ("sp_n_legs",2),("sp_chart_type","Line"),("sp_tf","1m"),
        ("sp_result",None),("sp_df",None),("sp_legs_live",[]),
        ("sp_live_hist",[]),("sp_live_on",False),("sp_last_tick",0.0),
        ("sp_last_err",None),
    ]:
...
def _ist_now():
    """Current time in IST as naive datetime (no tzinfo)."""
    return pd.Timestamp.now(tz="Asia/Kolkata").replace(tzinfo=None)

def _spread_now(legs):
    """
    Fetch live LTPs and return (spread_value, error_or_None).
    Uses a single batch quote call for speed.
    """
    from fyers_client import get_fyers_client, build_symbol
    # Build all symbols
    syms = []
    for leg in legs:
        try:
            syms.append(build_symbol(leg["index"], leg["expiry"], leg["cp"], leg["strike"]))
        except Exception:
            syms.append(None)

    # Batch quote — ONE API call for all legs
    ltp_map = {}
    try:
        fc = get_fyers_client()
        valid_syms = [s for s in syms if s]
        if valid_syms:
            resp = fc.quotes(data={"symbols": ",".join(valid_syms)})
            if resp.get("s") == "ok":
                for item in resp.get("d", []):
                    try:
                        ltp_map[item["n"]] = float(item["v"].get("lp", 0))
                    except Exception:
                        pass
    except Exception as e:
        return 0.0, f"Quote API error: {e}"

    total  = 0.0
    errors = []
    for leg, sym in zip(legs, syms):
        if sym and sym in ltp_map:
            sign   = 1 if leg["bs"] == "Buy" else -1
            total += sign * ltp_map[sym] * leg["ratio"]
        else:
            errors.append(f"{leg['index']} {leg['strike']}{leg['cp']}: not in response")

    err = "; ".join(errors) if errors else None
    return round(total, 2), err

# ─── live chart ───────────────────────────────────────────────────────────────
def _live_fig(hist, legs, chart_type):
    if not hist: return go.Figure()
    times  = [h[0] for h in hist]
    values = [h[1] for h in hist]
    last   = values[-1]; first = values[0]
    up_c   = "#26a69a"; dn_c = "#ef5350"
    line_c = up_c if last >= first else dn_c
    ...
    if chart_type == "Candlestick" and len(hist) >= 2:
        df = (pd.DataFrame({"t": times, "v": values})
              .set_index("t").resample("1min")["v"].ohlc().dropna().reset_index())
        if not df.empty:
    ...
    title = " / ".join(f"{l['index']} {l['strike']}{l['cp']} {l['bs'][0]}" for l in legs[:4])
    if times:
        fig.add_annotation(x=times[-1], y=last, text=f"  {last:+.2f}",
            showarrow=False, font=dict(size=12, color="#fff"),
            bgcolor=up_c if last >= 0 else dn_c, borderpad=4, xanchor="left")
    fig.update_layout(
        ...
        xaxis=dict(gridcolor="#1e222d", tickfont=dict(size=9, color="#787b86"),
                   ...
                   fixedrange=False,
                   # IST time display — times are already IST naive datetimes
                   tickformat="%H:%M:%S", nticks=8),
...
def _hist_fig(df, result, chart_type, tf):
    lgs   = result.get("legs", [])
    title = " / ".join(f"{l['index']} {l['strike']}{l['cp']} {l['bs'][0]}"
                       for l in lgs[:4]) + f"  [{tf}]"
    fig   = go.Figure()
    last  = float(df["close"].iloc[-1])
...
def _render_safety(legs):
    ...
    st.markdown(
        ...
        '🛡️ Safety Calculator'
        '<span style="font-size:11px;color:#787b86;margin-left:10px;padding:2px 8px;'
        'background:#1e222d;border:1px solid #2a2e39;border-radius:8px;">'
        'Auto-linked to legs above</span></div>', unsafe_allow_html=True)
    ...
    with sc_col:
        ...
        for i, leg in enumerate(legs):
            ...
            d = st.number_input(f"LEG {i+1} ({leg['index']}) step",
    ...
    with tbl_col:
        ...
        if build or _SS.get("sc_sig") != sig:
            ...
            for off in range(-n_rows, n_rows + 1):
                ...
                for i, leg in enumerate(legs):
                    ...
                    near  = _nearest(avail, leg["strike"] + off * diffs[i]) if avail else leg["strike"] + off * diffs[i]
                    row[f"LEG {i+1}"] = near
                    stk_row.append(near)
        ...
        th   = "".join(f'<th style="padding:5px 8px;font-size:10px;color:#787b86;'
                       f'text-align:center;border-bottom:1px solid #2a2e39;">{c}</th>' for c in hdr)
        ir   = "".join(f'<td style="padding:5px 8px;font-size:11px;font-weight:700;'
                       f'color:#ff9800;text-align:center;background:#1a1f2e;">{ivl.get(c,"")}</td>' for c in hdr)
        body = ""
        for off, row in mat:
            base = (off == 0)
            ...
            for c in hdr:
                v = row.get(c, "")
                if c == "SERIES":
                    fw = "700" if base else "400"; cc = "#2962ff" if base else "#787b86"
                    cells += f'<td style="padding:5px 8px;font-size:11px;font-weight:{fw};color:{cc};text-align:center;">{v}</td>'
                elif c.startswith("LEG"):
                    fw = "600" if base else "400"; cc = "#fff" if base else "#d1d4dc"
                    cells += f'<td style="padding:5px 8px;font-size:12px;font-weight:{fw};color:{cc};font-family:monospace;text-align:center;">{v}</td>'
                else:
                    try:
                        fv = float(v); cc = "#26a69a" if fv >= 0 else "#ef5350"; fw = "600" if base else "400"; txt = f"{fv:+.2f}"
                    except Exception:
                        cc = "#787b86"; fw = "400"; txt = str(v)
                    cells += f'<td style="padding:5px 8px;font-size:12px;font-weight:{fw};color:{cc};font-family:monospace;text-align:center;">{txt}</td>'
...
def render():
    _init()
    st.markdown('<div style="font-size:20px;font-weight:600;color:#d1d4dc;margin-bottom:4px;">📊 Spread Chart</div>',
                unsafe_allow_html=True)

    # ── Leg configuration ─────────────────────────────────────────────────────
    c1, c2, c3 = st.columns(3)
    with c1:
        n = st.selectbox("Legs", list(range(2,7)), index=_SS.sp_n_legs-2, key="sp_legs_sel")
        _SS.sp_n_legs = n
    with c2:
    ...
    for i in range(n):
        with leg_cols[i]:
            st.markdown(f'<div style="font-size:10px;color:#787b86;margin-bottom:6px;'
                        f'background:#2a2e39;padding:2px 8px;border-radius:10px;'
                        f'display:inline-block;">LEG {i+1}</div>', unsafe_allow_html=True)
            idx   = st.selectbox("Index", ["NIFTY","SENSEX","BANKNIFTY"],
                                 key=f"sp_idx_{i}", label_visibility="collapsed")
            exps, exp_err = _load_expiries(idx)
            if not exps:
                st.markdown(f'<div style="font-size:11px;color:#ff9800;">⏳ {exp_err or "loading…"}</div>',
                            unsafe_allow_html=True)
    ...
    # ═════════════════════════════════════════════════════════════════════════
    # LIVE FEED TAB
    # Pattern: fetch one tick → sleep → st.rerun()
    # Stop button works because rerun re-renders buttons BEFORE the sleep block.
    # ═════════════════════════════════════════════════════════════════════════
    with tab_live:
        st.markdown(
            f'<div style="font-size:11px;color:#787b86;margin-bottom:8px;">'
            f'Updates every <b style="color:#d1d4dc;">{REFRESH_INTERVAL}s</b> · '
            f'Live Fyers quotes · IST time · Works 24/7</div>', unsafe_allow_html=True)
        ...
        if _SS.sp_live_on:
            ...
            st.markdown(
                ...
                f'background:#0d2b1f;border:1px solid #26a69a40;border-radius:20px;'
                f'font-size:11px;color:#26a69a;">🟢 LIVE &nbsp;|&nbsp; '
                f'{n_ticks} ticks &nbsp;|&nbsp; {last_t} IST</div>', unsafe_allow_html=True)
        else:
            st.markdown(
                '<div style="display:inline-flex;align-items:center;gap:6px;padding:4px 10px;'
                'background:#1e222d;border:1px solid #2a2e3980;border-radius:20px;'
                'font-size:11px;color:#787b86;">⏹ STOPPED — click Start</div>',
                unsafe_allow_html=True)

        if _SS.get("sp_last_err"):
            st.warning(f"⚠️ {_SS.sp_last_err}")
        ...
        if _SS.sp_live_hist:
            ...
            st.markdown(
                f'<div style="font-size:32px;font-weight:700;color:{clr};'
                f'padding:4px 0;font-family:monospace;">{last_val:+.2f}</div>',
                unsafe_allow_html=True)
            st.plotly_chart(
                _live_fig(_SS.sp_live_hist, valid_legs, chart_type),
            ...
            st.markdown(
                ...
                'Click ▶ Start Live Feed</div></div>', unsafe_allow_html=True)

        # ── TICK: fetch price → sleep → rerun ──────────────────────────────
        # This block runs AFTER buttons are rendered so Stop works correctly.
        if _SS.sp_live_on:
            now     = time.time()
            elapsed = now - _SS.sp_last_tick

            if elapsed >= REFRESH_INTERVAL:
                val, err = _spread_now(valid_legs)
                ts = _ist_now()                          # IST timestamp — correct time
                _SS.sp_live_hist.append((ts, val))
                if len(_SS.sp_live_hist) > 7200:         # keep max 2 hours
                    _SS.sp_live_hist = _SS.sp_live_hist[-7200:]
                _SS.sp_last_tick = now
                _SS.sp_last_err  = err

            # Sleep remainder of interval then trigger next tick
            remaining = REFRESH_INTERVAL - (time.time() - _SS.sp_last_tick)
            if remaining > 0:
    ...
    # ═════════════════════════════════════════════════════════════════════════
    with tab_hist:
        st.markdown('<div style="font-size:11px;color:#787b86;margin-bottom:8px;">'
                    'Today\'s OHLCV candles from Fyers · Available during market hours '
                    '(9:15 AM – 3:30 PM IST)</div>', unsafe_allow_html=True)
        ...
        if _SS.sp_df is not None and _SS.sp_result is not None:
            ...
            for col, (lbl, val, clr) in zip(st.columns(5), [
                ...
                ("MAX P",   "∞" if r["max_profit"] is None else f"{r['max_profit']:.2f}", "#26a69a"),
                ...
                with col:
                    st.markdown(f'<div class="stat-chip"><div class="sc-label">{lbl}</div>'
                                f'<div class="sc-val" style="color:{clr};">{val}</div></div>',
                                unsafe_allow_html=True)
            st.plotly_chart(_hist_fig(_SS.sp_df, r, chart_type, tf),
                            use_container_width=True,
                            config={"scrollZoom":True,"displaylogo":False})
            df_s = pd.DataFrame(r["legs"])[["index","strike","expiry","cp","bs","ratio","ltp","net"]]
            df_s.columns = ["Index","Strike","Expiry","C/P","B/S","Ratio","LTP","Net"]
            ...
            st.markdown('<div style="height:160px;display:flex;align-items:center;'
                        ...
                        '<div style="font-size:13px;color:#787b86;">Click Calculate & Plot</div></div>',
                        unsafe_allow_html=True)
            ...
            with st.spinner("Fetching prices…"):
                ...
                for leg in valid_legs:
                    try:
                        ...
                        st.error(f"Leg {valid_legs.index(leg)+1} price error: {e}"); ok=False; break
            if ok:
                ...
                with st.spinner("Fetching candle history…"):
                    try:
                        _SS.sp_df = generate_spread_ohlcv(fresh, tf_minutes=TF_MAP[tf])
                        _SS.sp_result = dict(spread=round(spread,2), net_prem=round(net_prem,2),
                                             max_profit=max_p, max_loss=max_l, be=be, legs=fresh)
                        st.rerun()
                    except Exception as e:
                        st.error(f"Candle history failed: {e}\n\n"
                                 "Candles are only available during market hours. "
                                 "Use the 📡 Live Feed tab.")
        ...
        if show_g2 and any(l["ltp"]>0 for l in valid_legs):
            try:
                ...
                if _SS.sp_df is not None:
                    c2 = _SS.sp_df["close"].dropna()
                    if len(c2) >= 5:
                        hi_avg = round(c2.nlargest(5).mean(), 2)
                        lo_avg = round(c2.nsmallest(5).mean(), 2)
