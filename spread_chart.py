...
from fyers_client import get_live_quote, get_live_ltp, get_expiries
...
def _ist_now() -> pd.Timestamp:
    """Current time in IST as tz-naive Timestamp (fixes 6:25 AM UTC bug)."""
...
def _load_expiries(index):
    ...
    if _SS.get(ck):
        return list(_SS[ck].keys()), None
    try:
        return get_index_expiries(index), None
...
def _load_strikes(index, expiry):
    ...
    if _SS.get(ck):
        return _SS[ck], None
    try:
        return get_index_strikes(index, expiry), None
    except Exception as e:
        return [], str(e)


def _ensure_expiry_map(index: str):
    """Ensure expiry map is loaded — critical for build_symbol to work."""
    ck = f"expiries_{index}"
    if not _SS.get(ck):
        try:
            get_expiries(index)
...
def _spread_now(legs):
    """
    Fetch live LTPs via batch quote call.
    Ensures expiry maps loaded first so symbols resolve correctly.
    Returns (spread_value, error_or_None).
    """
    from fyers_client import get_fyers_client, build_symbol

    # Ensure all expiry maps are loaded
    for leg in legs:
        _ensure_expiry_map(leg["index"])

    # Build symbols
    syms = []
    for leg in legs:
        try:
            syms.append(build_symbol(leg["index"], leg["expiry"],
                                     leg["cp"], leg["strike"]))
        except Exception:
            syms.append(None)
    ...
    for leg, sym in zip(legs, syms):
        if sym and sym in ltp_map:
            ...
            errors.append(f"{sym} not in response")
...
def _live_fig(hist, legs, chart_type):
    ...
    fill_c = ("rgba(38,166,154,0.08)" if last >= first
               else "rgba(239,83,80,0.08)")
    ...
    title = " / ".join(
        f"{l['index']} {l['strike']}{l['cp']} {l['bs'][0]}"
        for l in legs[:4])
    if times:
        fig.add_annotation(
            x=times[-1], y=last, text=f"  {last:+.2f}",
            showarrow=False, font=dict(size=12, color="#fff"),
            bgcolor=up_c if last >= 0 else dn_c,
    ...
    fig.update_layout(
        ...
        xaxis=dict(
            ...
            fixedrange=False,
            tickformat="%H:%M:%S",   # IST — times are already IST naive
            nticks=8),
        yaxis=dict(
...
def _render_safety(legs):
    ...
    with tbl_col:
        sig = (str([(l["index"],l["strike"],l["expiry"],l["cp"])
                    for l in legs])
        ...
        if build or _SS.get("sc_sig") != sig:
            ...
            for off in range(-n_rows, n_rows + 1):
                ...
                for i, leg in enumerate(legs):
                    avail = _SS.get(
                        f"strikes_{leg['index']}_{leg['expiry']}", [])
                    near = (_nearest(avail, leg["strike"] + off * diffs[i])
                            if avail else leg["strike"] + off * diffs[i])
                    row[f"LEG {i+1}"] = near
                    stk_row.append(near)
                bid_t = ask_t = ltp_t = 0.0
                for i, leg in enumerate(legs):
                    try:
                        q    = get_live_quote(leg["index"], stk_row[i],
                                              leg["expiry"], leg["cp"])
                        sign = 1 if leg["bs"] == "Buy" else -1
        ...
        hdr = (["SERIES"]
               + [f"LEG {i+1}" for i in range(n_legs)]
               + ["BID","ASK","LTP"])
        ivl = {"SERIES":"STEP"}
        ...
        th = "".join(
            f'<th style="padding:5px 8px;font-size:10px;color:#787b86;'
            f'text-align:center;border-bottom:1px solid #2a2e39;">{c}</th>'
            for c in hdr)
        ir = "".join(
            f'<td style="padding:5px 8px;font-size:11px;font-weight:700;'
            f'color:#ff9800;text-align:center;background:#1a1f2e;">'
            f'{ivl.get(c,"")}</td>'
            for c in hdr)
        body = ""
        for off, row in mat:
            ...
            bl    = ("border-left:3px solid #2962ff;"
                     if base else "border-left:3px solid transparent;")
            cells = ""
            for c in hdr:
                v = row.get(c, "")
                if c == "SERIES":
                    ...
                    cells += (
                        ...
                        f'font-weight:{fw};color:{cc};'
                        f'font-family:monospace;text-align:center;">{v}</td>')
                else:
                    try:
                        fv  = float(v)
                    ...
                    cells += (
                        f'<td style="padding:5px 8px;font-size:12px;'
                        f'font-weight:{fw};color:{cc};'
                        f'font-family:monospace;text-align:center;">{txt}</td>')
            body += (
                f'<tr style="background:{bg};{bl}'
                f'border-bottom:1px solid #2a2e39;">{cells}</tr>')

        st.markdown(
            f'<div style="overflow-x:auto;border:1px solid #2a2e39;'
            f'border-radius:8px;">'
        ...
        if st.button("📥 Export Safety Matrix", key="sc_export"):
            ...
            with pd.ExcelWriter(buf, engine="openpyxl") as w:
                pd.DataFrame([r for _, r in mat]).to_excel(
                    w, index=False, sheet_name="Safety")
            buf.seek(0)
            st.download_button(
                "Download", data=buf,
...
def render():
    _init()
    st.markdown(
        '<div style="font-size:20px;font-weight:600;color:#d1d4dc;'
        'margin-bottom:4px;">📊 Spread Chart</div>',
        unsafe_allow_html=True)
    ...
    for i in range(n):
        with leg_cols[i]:
            ...
            # Ensure expiry map loaded for this index
            _ensure_expiry_map(idx)

            exps, exp_err = _load_expiries(idx)
            if not exps:
                st.markdown(
                    f'<div style="font-size:11px;color:#ff9800;">'
                    f'⏳ {exp_err or "loading…"}</div>',
                    unsafe_allow_html=True)
            ...
            # ATM default using live spot
            from fyers_client import get_spot_price as _spot
            try:
                spot_now = _spot(idx)
            except Exception:
                spot_now = {"NIFTY":22800,"SENSEX":82500,
                            "BANKNIFTY":48000}.get(idx, 22800)
            def_s = min(strikes, key=lambda x: abs(x - spot_now))
            cur   = _SS.get(f"sp_strike_{i}")
            didx  = (strikes.index(cur) if cur in strikes
                     else strikes.index(def_s))
            ...
            # Live LTP — expiry map is guaranteed loaded at this point
            ltp = 0.
            try:
                ...
                st.markdown(
                    ...
                    f'&nbsp;Net: <span style="color:{clr};">'
                    f'{signed:+.2f}</span></div>',
                    unsafe_allow_html=True)
            except Exception as e:
                st.markdown(
                    f'<div style="font-size:10px;color:#ef5350;margin-top:3px;">'
                    f'LTP error: {e}</div>',
                    unsafe_allow_html=True)
    ...
    st.markdown(
        ...
        f'color:{"#26a69a" if n_ready==n else "#ff9800"};">'
        f'✓ {n_ready}/{n} legs ready</div>',
        unsafe_allow_html=True)
    ...
    with tab_live:
        st.markdown(
            ...
            f' · Live Fyers quotes · IST time · Works 24/7</div>',
            unsafe_allow_html=True)
        ...
        if _SS.sp_live_on:
            ...
            last_t  = (_SS.sp_live_hist[-1][0].strftime("%H:%M:%S")
                       if _SS.sp_live_hist else "—")
            st.markdown(
                f'<div style="display:inline-flex;align-items:center;gap:6px;'
                f'padding:4px 10px;background:#0d2b1f;'
                f'border:1px solid #26a69a40;border-radius:20px;'
                f'font-size:11px;color:#26a69a;">'
                f'🟢 LIVE &nbsp;|&nbsp; {n_ticks} ticks '
                f'&nbsp;|&nbsp; {last_t} IST</div>',
                unsafe_allow_html=True)
        else:
            st.markdown(
                '<div style="display:inline-flex;align-items:center;gap:6px;'
                'padding:4px 10px;background:#1e222d;'
                'border:1px solid #2a2e3980;border-radius:20px;'
                'font-size:11px;color:#787b86;">'
                '⏹ STOPPED — click Start</div>',
                unsafe_allow_html=True)
        ...
        if _SS.sp_live_hist:
            ...
            st.markdown(
                ...
                f'padding:4px 0;font-family:monospace;">'
                f'{last_val:+.2f}</div>',
                unsafe_allow_html=True)
            st.plotly_chart(
        ...
        # Greeks
        show_g = st.checkbox("Show Net Greeks", value=False,
                              key="sp_greeks_live")
                ...
                for col, (lbl, val, clr) in zip(st.columns(5), [
                    ...
                    with col:
                        st.markdown(
                            ...
                            f'<div class="sc-val" style="color:{clr};">'
                            f'{val}</div></div>',
                            unsafe_allow_html=True)
            except Exception:
                pass
    ...
    with tab_hist:
        st.markdown(
            ...
            "Today's OHLCV candles from Fyers · "
        ...
        if _SS.sp_df is not None and _SS.sp_result is not None:
            ...
            for col, (lbl, val, clr) in zip(st.columns(5), [
                ...
                ("MAX L",    (f"{r['max_loss']:.2f}"
                              if r["max_loss"] else "—"), "#ef5350"),
                ("BE",       (f"{r['be']:.0f}"
                              if r["be"] else "—"), "#d1d4dc"),
            ]):
                with col:
            ...
            df_s = pd.DataFrame(r["legs"])[
                ["index","strike","expiry","cp","bs","ratio","ltp","net"]]
            df_s.columns = ["Index","Strike","Expiry","C/P",
                            "B/S","Ratio","LTP","Net"]
            ...
            st.markdown(
                ...
                'justify-content:center;background:#1e222d;'
                'border:1px solid #2a2e39;border-radius:8px;margin-bottom:10px;">'
                '<div style="font-size:13px;color:#787b86;">'
                'Click Calculate & Plot below</div></div>',
                unsafe_allow_html=True)

        show_g2 = st.checkbox("Show Net Greeks", value=False,
                               key="sp_greeks_hist")
        ...
        if st.button("⚡ Calculate & Plot", type="primary",
            ...
            with st.spinner("Fetching prices…"):
                ...
                for leg in valid_legs:
                    try:
                        ltp   = get_option_price(leg["index"], leg["strike"],
                                                  leg["expiry"], leg["cp"])
                        sign  = 1 if leg["bs"] == "Buy" else -1
                        fresh.append({**leg, "ltp": ltp,
                                      "net": round(sign*ltp*leg["ratio"],2)})
                    except Exception as e:
                        st.error(
                            f"Leg {valid_legs.index(leg)+1} price error: {e}")
                        ok = False
                        break
            ...
            if ok:
                ...
                if buys and sells:
                    ...
                    max_p = (sd - abs(spread)
                             if sd > abs(spread) else None)
                    max_l = abs(spread)
                    be    = (buys[0]["strike"] + spread
                             if buys[0]["cp"] == "CE"
                             else buys[0]["strike"] - spread)

                with st.spinner("Fetching candle history…"):
                    try:
                        _SS.sp_df     = generate_spread_ohlcv(
                            fresh, tf_minutes=TF_MAP[tf])
                        _SS.sp_result = dict(
