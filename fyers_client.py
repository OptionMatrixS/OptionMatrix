...
fyers_client.py
───────────────
Fyers API integration for Streamlit.
...
def _safe_json(r, step):
    if not r.text.strip():
        raise ValueError(f"{step}: empty response (HTTP {r.status_code})")
    try:
        return r.json()
    except Exception:
        raise ValueError(f"{step}: non-JSON (HTTP {r.status_code}): {r.text[:200]}")
...
def _generate_token():
    """
    Full TOTP auto-login. Returns (token, None) or (None, error_str).
    Steps 1-4 use the vagator/token endpoints.
    Step 5 uses generate_token() from fyers_apiv3 SessionModel.
    """
    cid      = _secret("FYERS_CLIENT_ID")
    secret   = _secret("FYERS_SECRET_KEY")
    username = _secret("FYERS_USERNAME")
    pin      = _secret("FYERS_PIN")
    totp_key = _secret("FYERS_TOTP_KEY")

    missing = [k for k,v in {"FYERS_CLIENT_ID":cid,"FYERS_SECRET_KEY":secret,
               "FYERS_USERNAME":username,"FYERS_PIN":pin,"FYERS_TOTP_KEY":totp_key}.items() if not v]
    if missing:
    ...
    try:
        ...
        d2  = _safe_json(r2, "Step2")
        if d2.get("s") != "ok":
            return None, f"Step2/TOTP failed: {d2} — check FYERS_TOTP_KEY is Base32 secret"

        # Step 3 — verify PIN  (try b64 first, fall back to sha256)
        for identifier in [_b64(pin), hashlib.sha256(pin.encode()).hexdigest()]:
            r3 = s.post("https://api-t2.fyers.in/vagator/v2/verify_pin_v2",
                        json={"request_key": d2["request_key"],
                              "identity_type": "pin",
                              "identifier": identifier}, timeout=15)
            d3 = _safe_json(r3, "Step3")
            if d3.get("s") == "ok":
                break
        else:
        ...
        sess = fyersModel.SessionModel(
            client_id=cid, secret_key=secret,
            redirect_uri=REDIRECT_URI,
            response_type="code", grant_type="authorization_code")
        sess.set_token(auth_code)
        d5    = sess.generate_token()
...
def _get_shared_token():
    # Option 1: direct token secret
    t = _secret("FYERS_ACCESS_TOKEN")
    if t and len(t) > 20:
        return t
    # Option 2: token file
    try:
        t = open("fyers_token.txt").read().strip()
        if t and len(t) > 20:
            return t
...
def get_fyers_client():
    ...
    client = fyersModel.FyersModel(client_id=_secret("FYERS_CLIENT_ID"),
...
def _fyers_sym_for(index):
    return _UNDERLYING_SYM.get(index.upper(), f"NSE:{index}-INDEX")

@st.cache_resource
def _fetch_expiry_codes(token, fyers_sym):
    try:
        fyers = fyersModel.FyersModel(client_id=_secret("FYERS_CLIENT_ID"),
                                      token=token, log_path="")
        resp  = fyers.optionchain(data={"symbol": fyers_sym,
...
def _days_to_expiry(label, index=""):
    try:
...
def get_strikes(index, expiry_label):
    ...
    try:
        resp = get_fyers_client().optionchain(
            data={"symbol": _fyers_sym_for(index), "strikecount": 50, "timestamp": ""})
        if not (resp and resp.get("s") == "ok"):
...
def build_symbol(index, expiry_label, cp, strike):
    ...
    if any(c.isalpha() for c in code):          # monthly
...
def _fetch_candles(symbol, interval=1, date_str=None):
    ...
    return df.drop(columns=["timestamp"]).set_index("time").pipe(
...
def _get_live_quote_fyers(symbol):
...
def get_iv_series_live(index, strike, expiry_label, cp, tf_minutes=5, date_str=None):
    ...
    for ts, row in df.iterrows():
        try:
            iv_pct = round(implied_volatility(row["close"],spot,strike,T,RISK_FREE_RATE,cp)*100,2)
        except Exception:
            iv_pct = 0.0
...
def get_multiplier_series_live(sx_strike, sx_expiry, n_strike, n_expiry,
    ...
    for idx, st_val, exp, cp in [
        ("SENSEX",sx_strike,sx_expiry,"CE"),("SENSEX",sx_strike,sx_expiry,"PE"),
