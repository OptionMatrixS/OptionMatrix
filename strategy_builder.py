...
def render():
    ...
    with ctrl[0]:
        index = st.selectbox("Index / Underlying", INDICES + ["Stock"], key="sb_index")
    with ctrl[1]:
        try:
            exps   = get_expiries(index if index in INDICES else "NIFTY")
    ...
    with ctrl[4]:
        try:
            spot = get_spot_price(index if index in INDICES else "NIFTY")
        except Exception:
    ...
    # ── Load strikes ───────────────────────────────────────────────────────────
    try:
        strikes = get_strikes(index if index in INDICES else "NIFTY", expiry)
    except Exception:
    ...
    for col,hdr in zip(cols_hdr,["Leg","B/S","CE/PE","Strike","Qty","Premium","IV%","Greeks"]):
        col.markdown(f'<div style="font-size:10px;color:#787b86;font-weight:600;">{hdr}</div>',
                     unsafe_allow_html=True)

    legs = []
    for i in range(n_legs):
        # Apply preset defaults if preset selected
        ...
        with leg_cols[0]:
            ...
            st.markdown(f'<div style="padding-top:8px;font-size:11px;font-weight:600;color:{color};">L{i+1}</div>',
                        unsafe_allow_html=True)
        with leg_cols[1]:
        ...
        with leg_cols[3]:
            ...
            didx = strikes.index(cur) if cur in strikes else strikes.index(def_strike)
        ...
        with leg_cols[5]:
            # Fetch live premium
            try:
                live_prem = get_live_ltp(index if index in INDICES else "NIFTY",
                                          strike, expiry, cp)
            except Exception:
                live_prem = 0.0
            prem = st.number_input("", min_value=0.0, value=float(round(live_prem,2)),
                                   step=0.5, key=f"sb_prem_{i}",
                                   label_visibility="collapsed",
                                   help="Auto-filled from live price. Edit to override.")
        with leg_cols[6]:
            # Calculate IV
            T = _dte(expiry, index if index in INDICES else "NIFTY")
            try:
                iv = implied_volatility(prem, spot, strike, T, RISK_FREE_RATE, cp) * 100
                st.markdown(f'<div style="padding-top:8px;font-size:12px;color:#ff9800;">{iv:.1f}%</div>',
                            unsafe_allow_html=True)
            except Exception:
                st.markdown('<div style="padding-top:8px;color:#787b86;">—</div>', unsafe_allow_html=True)
                iv = 0.
        with leg_cols[7]:
            # Show delta
            try:
