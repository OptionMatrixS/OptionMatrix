...
# Add the app's own directory to path — works whether files are flat or in pages/
APP_DIR = os.path.dirname(os.path.abspath(__file__))
if APP_DIR not in sys.path:
    sys.path.insert(0, APP_DIR)

# Also add pages/ subfolder if it exists
...
def load_page(name):
    """Load a page module whether it's in pages/ or root folder."""
    return importlib.import_module(name)

spread_chart      = load_page("spread_chart")
iv_calculator     = load_page("iv_calculator")
multiplier_chart  = load_page("multiplier_chart")
admin_panel       = load_page("admin_panel")
safety_calculator = load_page("safety_calculator")
...
with st.sidebar:
    ...
    if role == "admin" or "safety"     in tools: nav_items.append(("🛡️", "Safety Calculator", "safety"))
    if role == "admin" or "iv"         in tools: nav_items.append(("🌡️", "IV Calculator",     "iv"))
    if role == "admin" or "multiplier" in tools: nav_items.append(("✖️", "Multiplier Chart",  "multiplier"))
    ...
    if st.button("🚪  Logout", use_container_width=True, type="secondary"):
        ...
        st.session_state.sp_legs_live   = []
        st.session_state.page           = "spread"
        st.rerun()

    st.markdown(
        '<div style="font-size:10px;color:#2a2e39;text-align:center;padding-top:20px;">Option Matrix v1.1</div>',
        unsafe_allow_html=True
    )
...
elif page == "safety":     gate("safety",     safety_calculator.render)
