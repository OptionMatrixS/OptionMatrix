...
def render():
    _init()
    st.markdown('<div style="font-size:20px;font-weight:600;color:#d1d4dc;margin-bottom:4px;">🌡️ IV Calculator</div>',
                unsafe_allow_html=True)
    st.markdown('<div style="font-size:12px;color:#787b86;margin-bottom:16px;">Implied Volatility — Black-Scholes. Up to 5 expiries as individual lines.</div>',
                unsafe_allow_html=True)
    ...
    with ctrl_col:
        ...
        st.markdown('<div class="sec-header" style="margin-top:8px;">Select Expiries</div>',
                    unsafe_allow_html=True)
        selected = []
        for i in range(n_exp):
            e = st.selectbox(f"Expiry {i+1}", exps,
                             index=min(i, len(exps)-1), key=f"iv_exp_{i}")
            selected.append(e)
        ...
        if st.button("📈  Calculate IV", use_container_width=True, type="primary"):
            with st.spinner("Fetching live IV data..."):
                try:
                    series = {}
                    for exp in selected:
                        series[exp] = get_iv_series(idx, strike, exp, cp,
                                                     tf_minutes=TF_MAP[tf])
                    _SS.iv_result = dict(index=idx, strike=strike, cp=cp,
                                         tf=tf, expiries=selected, series=series)
                    st.rerun()
        ...
        if _SS.iv_result:
            r = _SS.iv_result
            st.markdown("---")
            st.markdown('<div class="sec-header">Current IV</div>', unsafe_allow_html=True)
            for i, exp in enumerate(r["expiries"]):
                ...
                st.markdown(
                    ...
                    f'font-family:\'JetBrains Mono\',monospace;">{latest:.2f}%</div>'
    ...
    with chart_col:
        if not _SS.iv_result:
            st.markdown('<div style="height:420px;display:flex;align-items:center;justify-content:center;'
                        ...
                        '<div style="text-align:center;font-size:14px;color:#787b86;">Configure and click Calculate IV</div>'
        ...
        for i, exp in enumerate(r["expiries"]):
            ...
            fig.add_trace(go.Scatter(
                x=df["time"], y=df["iv_pct"], mode="lines",
                name=f"{exp}  {latest:.2f}%",
                line=dict(color=color, width=1.8, dash=dash),
                hovertemplate=f"<b>{exp}</b><br>IV: %{{y:.2f}}%<extra></extra>"))
            fig.add_annotation(x=df["time"].iloc[-1], y=latest,
        ...
        fig.update_layout(
            ...
            yaxis=dict(gridcolor="#1e222d", tickfont=dict(size=10,color="#787b86",family="JetBrains Mono"),
                       ...
                       title=dict(text="IV %",font=dict(size=11,color="#787b86")), ticksuffix="%"),
        ...
        st.markdown('<div class="sec-header" style="margin-top:8px;">IV Summary</div>',
                    unsafe_allow_html=True)
        ...
        for col,(exp,color) in zip(cols,zip(r["expiries"],[EXPIRY_COLORS[i] for i in range(len(r["expiries"]))])):
            ...
            with col:
                st.markdown(
                    ...
                    f'H:{iv.max():.2f}% L:{iv.min():.2f}%</div></div>',
                    unsafe_allow_html=True)
