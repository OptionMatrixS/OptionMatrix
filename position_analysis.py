...
def _read_file(uploaded) -> pd.DataFrame:
    """
    Read uploaded file with robust encoding detection.
    Handles UTF-8, Windows-1252 (cp1252), latin-1, and ISO-8859-1
    which are all common in broker-exported CSV files.
    """
    name = uploaded.name.lower()
    raw  = uploaded.read()  # read bytes once

    if name.endswith(".csv"):
        # Try encodings in order of likelihood for Indian broker exports
...
def render():
    ...
    # ── Position table ────────────────────────────────────────────────────────
    st.markdown(
        '<div class="sec-header">Position Table  '
        '<span style="font-weight:400;color:#2a2e39;">— tick rows, then send to Spread Chart</span>'
        '</div>', unsafe_allow_html=True)
    ...
    for _, row in df.iterrows():
        ...
        for ci, col_name in enumerate(cols_show):
            ...
            if col_name == "Scrip Type":
                ...
                cell = (f'<div style="font-size:11px;color:{clr};font-weight:600;'
                        f'background:{bg};padding:4px 2px;border-bottom:1px solid #2a2e39;">{val}</div>')
            elif col_name in NUMERIC_COLS:
                clr  = _color(val)
                cell = (f'<div style="font-size:11px;color:{clr};background:{bg};'
                        f'padding:4px 2px;border-bottom:1px solid #2a2e39;'
                        f'font-family:monospace;text-align:right;">{_fmt(val)}</div>')
            elif col_name == "Expiry Date":
                try:    v = pd.to_datetime(val).strftime("%d %b %y")
                except: v = str(val)
                cell = (f'<div style="font-size:11px;color:#d1d4dc;background:{bg};'
                ...
                cell = (f'<div style="font-size:11px;color:#d1d4dc;font-weight:500;background:{bg};'
                        f'padding:4px 2px;border-bottom:1px solid #2a2e39;'
                        f'font-family:monospace;text-align:center;">{sv}</div>')
            else:
                cell = (f'<div style="font-size:11px;color:#d1d4dc;background:{bg};'
                        f'padding:4px 2px;border-bottom:1px solid #2a2e39;">{val}</div>')
            row_cols[ci+1].markdown(cell, unsafe_allow_html=True)
    ...
    try:
        ...
        bep_total    = abs(bep_total) if ref_sign > 0 else (-abs(bep_total) if ref_sign < 0 else bep_total)
    ...
    total_cols[0].markdown(
        ...
        f'border-top:2px solid #2962ff;text-align:center;">{len(df)}</div>',
        unsafe_allow_html=True)
    for ci, col_name in enumerate(cols_show):
        if col_name == "ID":
            ...
            total_cols[ci+1].markdown(
                f'<div style="font-size:11px;font-weight:700;color:{clr};'
                f'border-top:2px solid #2962ff;padding:5px 2px;'
                f'font-family:monospace;text-align:right;">{_fmt(tot)}</div>',
                unsafe_allow_html=True)
        else:
            total_cols[ci+1].markdown(
    ...
    with sc1:
        ...
        st.markdown(
            ...
            f'<div style="font-size:22px;font-weight:600;color:{checked_color};">{n_checked}</div>'
    ...
    with sc2:
        ...
        if n_checked > 6:
            st.warning(f"⚠️ Max 6 legs. You selected {n_checked}. Tick ≤ 6 rows.")
        elif n_checked == 0:
            st.info("Tick rows above to select positions.")

        sb1, sb2 = st.columns(2)
        with sb1:
            if st.button(f"📊  Send {n_checked} to Spread Chart",
                         use_container_width=True, type="primary",
                         disabled=btn_disabled, key="pos_send_to_spread"):
                ...
                if legs:
                    ...
                    st.rerun()
        with sb2:
            if st.button(f"🏗️  Send {n_checked} to Strategy Builder",
                         use_container_width=True, type="secondary",
                         disabled=btn_disabled, key="pos_send_to_sb"):
    ...
    with sc3:
        if n_checked > 0:
            ...
            for _, row in checked_rows.head(6).iterrows():
                und = row.get("Underlying",""); stk = row.get("Strike Price","")
    ...
    if _SS.pos_groups:
        for grp_name, row_ids in list(_SS.pos_groups.items()):
            ...
            if grp_df.empty:
                del _SS.pos_groups[grp_name]; continue
            ...
            with st.expander(
                f"📁  {grp_name}  ·  {len(grp_df)} positions  ·  MTM: {_fmt(grp_mtm)}",
                expanded=False):
                gm1,gm2,gm3,gm4 = st.columns(4)
                for gcol,(lbl,val,clr) in zip([gm1,gm2,gm3,gm4],[
                ...
                show_cols_grp = [c for c in ["Underlying","Expiry Date","Strike Price",
                                              "Scrip Type","Net Position CF","MTM","LTP"]
                                 if c in grp_df.columns]
                grp_disp = grp_df[show_cols_grp].copy()
                if "Expiry Date" in grp_disp.columns:
                    grp_disp["Expiry Date"] = pd.to_datetime(
                        grp_disp["Expiry Date"], errors="coerce").dt.strftime("%d %b %y")
                st.dataframe(grp_disp, use_container_width=True, hide_index=True, height=200)
                ...
                with ba1:
                    if st.button(f"📊 Send to Spread Chart",
                        ...
                        if legs:
                            n = len(legs)
                            _SS.sp_n_legs = n; _SS.sp_legs_live = legs
                            for i, leg in enumerate(legs):
                                _SS[f"sp_idx_{i}"]    = leg["index"]
                                _SS[f"sp_strike_{i}"] = leg["strike"]
                                _SS[f"sp_exp_{i}"]    = leg["expiry"]
                                ...
                                _SS[f"sp_bs_{i}"]     = leg["bs"]
                                _SS[f"sp_ratio_{i}"]  = leg["ratio"]
                            _SS.sp_result = None; _SS.sp_df = None
                            _SS.page = "spread"; st.rerun()
                with ba2:
                ...
                with ba3:
                    if st.button(f"🗑️ Delete group",
                        ...
                        del _SS.pos_groups[grp_name]; st.rerun()
    else:
        st.markdown(
            ...
            'No groups yet. Tick rows, enter a name, and click Create group.</div>',
            unsafe_allow_html=True)
